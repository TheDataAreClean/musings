...
Covers: every post, the home page, the three section indices (`/ideas/`, `/notes/`, `/snaps/` — with their real `postSigil`), every tag page (`/tags/{tag}/` — title highlighted like the real page's `<mark>{{ tag }}</mark>`, description a live-computed `"N posts tagged as {tag}."`), and About. Deliberately not covered: the 404 page (not worth a card) and the feed (XML, no meta tags).
